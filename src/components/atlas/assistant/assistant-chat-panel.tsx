...
type ChatActionResult<T> = { ok: true; data: T } | { ok: false; error: string };
...
type AssistantChatPanelProps = {
  /** Translation namespace — mirrors "assistant"/"adminAssistant"'s exact key shape (see messages/en.json). */
  namespace: string;
  historyAction: () => Promise<ChatActionResult<AssistantMessageDto[]>>;
  sendAction: (input: { text: string }) => Promise<ChatActionResult<AssistantMessageDto[]>>;
};

/**
 * Shared presentational chat panel behind both the client-portal
 * AssistantChat and the admin-console AdminAssistantChat — identical UI
 * (floating button, history load, composer, error mapping) for both
 * surfaces; only the server actions and copy differ, so this is the one
 * place that state/scroll/error-mapping logic lives.
 */
export function AssistantChatPanel({ namespace, historyAction, sendAction }: AssistantChatPanelProps) {
  ...
  }, [open, historyLoaded, t, historyAction]);
